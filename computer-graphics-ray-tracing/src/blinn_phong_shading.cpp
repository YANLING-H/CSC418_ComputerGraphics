...
{
  Ray ray_to_light;   // Ray from point of intersection of object to light source
  ray_to_light.origin = ray.origin + t * ray.direction;
  double max_t;

  // Variables to capture outputs from first_hit
  int new_hit_id;
  double new_t;
  ...
  for (int i = 0; i < lights.size(); ++i) {
    lights[i]->direction(ray_to_light.origin, ray_to_light.direction, max_t); // Set ray to point to light source
